...
uv run dtcwt-noise -v akiyo --sigma 0 10 -b 100 300 500
```
...
# 전체 비디오
uv run dtcwt-noise -v akiyo foreman mobile stefan
...
uv run dtcwt-noise -v akiyo --sigma 0 10 --no-include_precodec_ablation
```
...
```bash
# 에지맵 기반 품질 분석
uv run python -m dtcwt_video.edge_analysis

# 잔차 증폭 시각화 (전처리 효과 육안 확인)
uv run python -m scripts.visualize_residuals \
    -o videos/akiyo.y4m \
...
### Reliable Metric Summary (Proposed vs Baseline, 4개 CIF 영상 평균)

| 조건 | ΔPSNR-Y | ΔMS-SSIM | ΔPSNR-B | ΔEPSNR | 해석 |
|---|---|---|---|---|---|
| Clean (σ=0) | −0.08 dB | −0.0002 | −0.02 dB | −0.03 dB | 전처리 오버헤드 |
| Noisy (σ=5) | +0.37 dB | +0.0018 | +0.35 dB | +0.32 dB | 소폭 개선 |
| Noisy (σ=10) | **+0.68 dB** | **+0.0074** | **+0.84 dB** | **+0.32 dB** | 명확한 개선 |
| Noisy (σ=15) | **+1.07 dB** | **+0.0172** | **+1.53 dB** | **+0.35 dB** | 최대 개선 |
